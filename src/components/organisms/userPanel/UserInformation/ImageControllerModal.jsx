...
const ImageControllerModal = ({ isOpen, setIsOpen }) => {
  ...
  const { mutate: addUserProfileMutate } = useMutation({
    mutationFn: (value) =>
      toast.promise(addUserProfileImage(value), {
        pending: "در حال بروزرسانی عکس ها",
        success: {
          render({ data }) {
            return data.data.message;
          },
        },
        error: "در ارسال عکس مشکلی پیش آمد.",
      }),
    onSuccess: (result) => {
  ...
  const { mutate: selectUserProfileMutate } = useMutation({
    mutationFn: (value) =>
      toast.promise(selectUserProfileImage(value), {
        pending: "در حال انتخاب عکس مورد نظر",
        success: {
          render({ data }) {
            return data.data.message;
          },
  ...
  const { mutate: deleteUserProfileMutate } = useMutation({
    mutationFn: (value) =>
      toast.promise(deleteUserProfileImage(value), {
        pending: "در حال حذف عکس مورد نظر",
        success: {
          render({ data }) {
            return data.data.message;
          },
